...
class ImagesProvider extends ChangeNotifier {
  ...
  Future<List<Image>> getListImg(List<String> urls) async {
    ...
    /// data from firstImg and secondImg will be changed below
    ...
    int wb1 = firstImg.getWhiteBalance();
    int wb2 = secondImg.getWhiteBalance();
    

    ///  10 - delta
    final bool isEqualWB = (wb1 - wb2).abs() < 10;
    ...
    if ((wb1 - wb2).abs() < 10) {
      ...
      wb1 = firstImg.getWhiteBalance();
      wb2 = secondImg.getWhiteBalance();
    }
      ...
      for (int j = 0; j < height; j++) {
        ...
        if (isEqualWB) {
          if ((rgb1[0] - rgb2[0]).abs() >
              (img.getLuminance(rgb1[0]) - img.getLuminance(rgb2[0])).abs() +
                  (wb1 - wb2).abs() + 2)
          ...
          if ((rgb1[0] - rgb2[0]).abs() >
              ((wb1 - wb2).abs() -
                  (img.getLuminance(rgb1[0]) - img.getLuminance(rgb2[0])).abs()) ||
              (rgb1[1] - rgb2[1]).abs() >
                  (wb1 - wb2).abs() - ((img.getLuminance(rgb1[1]) - img.getLuminance(rgb2[1])).abs()) ||
              (rgb1[2] - rgb2[2]).abs() >
                  (wb1 - wb2).abs() - ((img.getLuminance(rgb1[2]) - img.getLuminance(rgb2[2])).abs())) {
